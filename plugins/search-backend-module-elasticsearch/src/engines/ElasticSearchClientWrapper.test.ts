...
import { OpenSearchElasticSearchClientOptions } from './ElasticSearchClientOptions';
...
describe('ElasticSearchClientWrapper', () => {
  ...
  describe('openSearchClient', () => {
    ...
    beforeEach(async () => {
      osOptions = await createElasticSearchClientOptions(
        new ConfigReader({
          provider: 'aws',
          node: 'https://my-es-cluster.eu-west-1.es.amazonaws.com',
        }),
      );
      (osOptions as OpenSearchElasticSearchClientOptions).connection =
        mock.getConnection();
      jest.clearAllMocks();
    });
